...
impl fmt::Display for Message {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::UserJoined(content) => write!(f, "UserJoined({})", content),
            Self::UserLeft(content) => write!(f, "UserLeft({})", content),
            Self::Chat { sender, content } => write!(f, "Chat {{ sender: {}, content: {} }}", sender, content),
        }
    }
}
    ...
    async fn broadcast(&self, addr: SocketAddr, message: Arc<Message>) {
        for peer in self.peers.iter() {
            ...
            if let Err(e) = peer.value().send(message.clone()).await {
                ...
                // if send failed, peer might be gone, remove it from the state
